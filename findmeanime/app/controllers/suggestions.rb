#THIS IS A SET OF FUNCTIONS RELATED TO THE LOGIC OF THE PROGRAM, NOTABLY THE SUGGESTIONS ENGINE.  There are written as helper functions for get_suggestions, however, they probably have greater scope than that
...
#first, create a formatted list of shows to look through
  ...
  while(user.watched_animes[i].anime)
    j=0;
    while(shows[j,0])
      #calculate the percentage of users who have seen my_shows[i] who have liked, disliked, and are neutral about shows[j]
      numliked = certain_opinion(user.watched_animes[i].anime, shows[j,0], user.watched_animes[j].rank, 1).length;
      numhated = certain_opinion(user.watched_animes[i].anime, shows[j,0], user.watched_animes[j].rank, -1).length;
      numneutral = certain_opinion(user.watched_animes[i].anime, shows[j,0], user.watched_animes[j].rank, 0).length;
      total = numliked + numhated + numneutral;
      #calculate the portion of the rating of shows[j] generated by my_shows[i]
...
#removes shows from all_shows that are in my_shows in order to not suggest already watched programs.  note that my_shows is a 
...
def shared_user(show1, show2)
  i = 0
  while(all_users[i])
    if(all_users[i].watched_animes.includes_show?(show1) and all_users[i].watched_animes.includes_show?(show2))
      return all_users[i]
    end
  end
  return nil;
